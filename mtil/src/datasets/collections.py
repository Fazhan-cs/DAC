...
class ClassificationDataset:
    ...
    @property
    def template(self):
        random_index = random.randint(0, len(self.templates) - 1)
        #random_index=0
        if self.append_dataset_name_to_template:
            return lambda x: self.templates[random_index](x)[:-1] + f", from dataset {self.name}]."
        return self.templates[random_index]
...
class TinyImagenet(ClassificationDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.classnames = list(self.train_dataset.class_to_label.values())
...
class TinyImageNet_dataset(Dataset):
    def __init__(self, root, train=True, transform=None):
       ...
       # self.dataset_train = TinyImageNet200(root, train=True, transform=None)
        if (self.Train):
